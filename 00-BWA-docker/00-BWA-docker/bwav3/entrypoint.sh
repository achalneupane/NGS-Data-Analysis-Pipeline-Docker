...
## v2.2 update - the previous version was not running with single FQ scripts; because the ${TIMING} card was not defined inside the loop - line 305; so I added "if [ ! -z "${TIMING}" ]; then TIMING=(/usr/bin/time -v); fi: at line 304

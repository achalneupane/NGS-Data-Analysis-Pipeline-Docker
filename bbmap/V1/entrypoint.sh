...
if [ -z "${INDIR}" ]; then echo "ERROR, must provide INDIR variable \${INDIR}"; quit "Job Config"; fi
if [ -z "${OUTDIR}" ]; then echo "ERROR, must provide OUTDIR variable \${OUTDIR}"; quit "Job Config"; fi
...
"${TIMING[@]}" "${CUR_STEP}" -Xmx${MEM}g in="${INDIR}/${sortedFQ}" delimiter=${DELIM} prefixmode=f column=${COLNUM} out="${OUTDIR}/${sampleFQ}_%_#.fq"
exitcode=$?

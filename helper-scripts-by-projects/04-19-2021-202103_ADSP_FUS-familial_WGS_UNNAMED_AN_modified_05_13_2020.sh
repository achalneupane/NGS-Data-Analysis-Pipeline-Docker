...
LOOKUP="${PR}-MGIID-sm-dna-pr-rgbase-cram-smdir-cramloc.csv"
...
((Snumber=Snumber+1)); \
	...
	CRAM="$(grep "${SM},${DNA},${PR}" "${LOOKUP}" | cut -d, -f${LOOKUP_COL_CRAMFILE})"
	export OUT_DIR="${BASE}/WXS_Aquilla/01-RAW/${PR}/${FULLSM}"; \
	INPUT_LIST=(); \
	WAIT_LIST=(); \
	rsync -avh ${USER}@fenix.psych.wucon.wustl.edu:${CRAMLOC}/${CRAM} ${OUT_DIR}/; \
	rsync -avh ${USER}@fenix.psych.wucon.wustl.edu:${CRAMLOC}.${CRAM}.crai ${OUT_DIR}/; \
	export MEM=16; \
	...
	echo -e "OUTPUT is in "${BASE}/WXS_Aquilla/01-RAW/${PR}/${FULLSM}/${CRAMBASE}_s00rvtcram.%J" \n OUTDIR is ${OUT_DIR}";\
	bsub \
	-J "${FULLSM}_s00rvtcram" \
	-o "${BASE}/WXS_Aquilla/01-RAW/${PR}/${FULLSM}/${CRAMBASE}_s00rvtcram.%J" \
	-u "${EMAIL}" \
	-n1 -W 1440 \
	-R "rusage[mem=18192]" \
	-q research-hpc \
	...
	/bin/bash; \
	echo -e "00b - Starting revertcram for ${FULLSM}\n${CRAMFILE}\n${SM}\n${DNA}\n${PR}"; \

...
if [ "${RUN_TYPE}" = "genome" ]; then
	...
	&& "${TIMING[@]}" samtools view -b -T ${REF} -o "${JOB_TMP}/${BAMFILENAME%.*}.aln.srt.bam" "${JOB_TMP}/${CRAMFILE}"
	exitcode="$?"
	end=$(${DATE}); echo "[$(display_date ${end})] cramtobam for ${RUN_TYPE} finished, exit code: ${exitcode}, step time $(date_diff ${start} ${end})"
	
	if [ ${exitcode} -eq 0 ]; then
	mv -vi "${JOB_TMP}/${BAMFILENAME%.*}.aln.srt.bam" "${OUTDIR}/" \
		&& mv -fv "${JOB_TMP}/${CRAMFILENAME}" "${RAW_DIR}/" \
		&& rmdir -v "${JOB_TMP}" \
		&& echo "After processing, CRAMFILE is located at ${RAWDIR} and BAMFILE is located at ${OUTDIR} and ready for Validate Same "
	else
	...
	# Output is staged in ${JOB_TMP} and moved to ${OUTDIR} only once complete (to complement with other automation workflows)
	mv -vi "${CRAMFILE}" "${JOB_TMP}" \
	&& "${TIMING[@]}" samtools view -b -T "${REF}" "${JOB_TMP}/${CRAMFILE}" | ${BEDTOOLS_DIR}/bedtools intersect -abam stdin -b "${PADDED_BED}" > "${JOB_TMP}/${BAMFILENAME%.*}.aln.srt.isec-paddedexome.bam"
	exitcode="$?"

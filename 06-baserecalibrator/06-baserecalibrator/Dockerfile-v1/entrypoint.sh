...
VERSION="0.1"
GATK_THREADS=1
# Entrypoint script for docker container achalneupane/baserecal:${VERSION}
...
#   BAMFILE     .bam input file on which to perform Base Recalibration
...
# DBSNP1="${BASE}/GATK_pipeline/Reference/dbsnp_138.b37.excluding_sites_after_129.vcf"
# DBSNP2="${BASE}/GATK_pipeline/Reference/dbsnp_138.b37.vcf"
...
if [ -z "${BAMFILE}" ]; then echo "ERROR, must provide .sam or .bam file in variable \${BAMFILE}"; quit "Input File"; fi

if ! [ -r "${BAMFILE}" ]; then echo "ERROR, cannot read ${BAMFILE}"; echo "Please make sure you provide the full path"; quit "Input File"; fi
...
"${TIMING[@]}" java ${JAVAOPTS} -jar "${GATK360}" \
    ...
    -knownSites ${DBSNP1} \
	-knownSites ${DBSNP2} \
    -knownSites ${MILLS_GOLD} \
    -knownSites ${ONEKG_INDELS} \
    ${GATK_PARAMS} \
    -o "${BAMFILE%.bam}.recal.table"
exitcode=$?
